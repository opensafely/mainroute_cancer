...
from dataset_definition_fit import make_dataset_fit

index_date = "2018-03-23"
end_date = "2023-10-22"

dataset = make_dataset_fit(index_date="2018-03-23", end_date="2023-10-22")
...
dataset.num_fit = clinical_events.where(clinical_events.snomedct_code.is_in(codelists.fit_codes)
        ).where(
            clinical_events.date.is_on_or_between(index_date, end_date)
        ).where(
...
fit_positive_1 = clinical_events_ranges.where(clinical_events_ranges.snomedct_code.is_in(codelists.fit_codes)
        ).where(
            clinical_events_ranges.date.is_on_or_between(index_date, end_date)
        ).where(
            clinical_events_ranges.numeric_value>=10
        ).where(
            clinical_events_ranges.comparator!="<"
        ).exists_for_patient()

fit_postive_2 = clinical_events_ranges.where(clinical_events_ranges.snomedct_code.is_in(codelists.fit_codes)
        ).where(
            clinical_events_ranges.date.is_on_or_between(index_date, end_date)
        ).where(
            clinical_events_ranges.snomedct_code=="389076003"
        ).exists_for_patient()

fit_positive_3 = clinical_events_ranges.where(clinical_events_ranges.snomedct_code.is_in(codelists.fit_codes)
        ).where(
            clinical_events_ranges.date.is_on_or_between(index_date, end_date)
        ).where(
            clinical_events_ranges.snomedct_code=="59614000"
        ).exists_for_patient()

dataset.fit_test_positive = (fit_positive_1 | fit_postive_2 | fit_positive_3)