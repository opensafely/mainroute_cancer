...
unique_dates = data_referral_conversion['symptom_date'].unique()
...
for item in unique_dates:
    df = data_referral_conversion.loc[data_referral_conversion['symptom_date'] == item]
    con = df['conversion'].mean()
    dict.update({item: con})

plt.bar(range(len(dict)), list(dict.values()), tick_label=list(dict.keys()))

plt.savefig('output/colorectal_cancer_6.jpg')